...
RETENTION_SCALE = [
    (Decimal("0.00"), Decimal("0.00"), Decimal("12450.00"), Decimal("0.19")),
    (Decimal("12450.00"), Decimal("2365.50"), Decimal("7750.00"), Decimal("0.24")),
    (Decimal("20200.00"), Decimal("4225.50"), Decimal("15000.00"), Decimal("0.30")),
    (Decimal("35200.00"), Decimal("8725.50"), Decimal("24800.00"), Decimal("0.37")),
    (Decimal("60000.00"), Decimal("17901.50"), Decimal("240000.00"), Decimal("0.45")),
    (Decimal("300000.00"), Decimal("125901.50"), None, Decimal("0.47")),
]
...
def scale_tax(base: Decimal) -> Decimal:
    if base <= 0:
        return Decimal("0.00")
    for start, quota, width, rate in RETENTION_SCALE:
        if width is None or base <= start + width:
            return quota + ((base - start) * rate)
    return Decimal("0.00")
...
def calculate_irpf_2026(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if exempt or retrib <= 0:
        ...
        if child_support_annuity > 0 and base - child_support_annuity > 0:
            quota_1 = scale_tax(base - child_support_annuity) + scale_tax(child_support_annuity)
            quota_2 = scale_tax(minimum_personal_family + Decimal("1980.00"))
        else:
            quota_1 = scale_tax(base)
            quota_2 = scale_tax(minimum_personal_family)
