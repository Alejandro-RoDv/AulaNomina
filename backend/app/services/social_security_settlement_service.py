...
def _sum_items_by_markers(payroll: Payroll, markers: tuple[str, ...]) -> Decimal:
    ...
    for item in getattr(payroll, "items", []) or []:
        if any(marker in _item_marker(item) for marker in markers):
...
def build_line_values(payroll: Payroll) -> dict[str, Decimal]:
    ...
    stored_company_total = money(payroll.company_total_social_security or 0)
    company_total = stored_company_total or company_component_total
    ...
    return {
        ...
        "company_total": money(company_total),
        "bonuses": bonuses,
...
def _validate_line(
    ...
    if not employee:
        issues.append(
            _issue(
                "employee_id",
                "EMPLOYEE_MISSING",
                "La nómina no tiene trabajador vinculado.",
        ...
        issues.append(
            _issue(
                "naf",
                "NAF_REQUIRED",
                "Falta el número de afiliación del trabajador.",
                payroll_id=payroll.id,
            )
        )
    ...
    if not contract:
        issues.append(
            _issue(
                "contract_id",
                "CONTRACT_MISSING",
                "La nómina no tiene contrato vinculado.",
                payroll_id=payroll.id,
            )
        )
    ...
    if resolve_payroll_ccc(payroll) != selected_ccc:
        issues.append(
            _issue(
                "ccc_id",
    ...
    if contribution_days < 0 or contribution_days > 31:
        issues.append(
            _issue(
                "contribution_days",
    ...
    elif contribution_days == 0 and any(
        values[field] > 0
        for field in (
            "common_contingencies_base",
            "professional_contingencies_base",
        ...
        if values[field] < 0:
            issues.append(
                _issue(
                    ...
                    "La base de cotización no puede ser negativa.",
                    payroll_id=payroll.id,
                )
            )

    payroll_status = str(getattr(payroll, "status", "") or "").lower()
    if payroll_status in {"draft", "pending"}:
        issues.append(
            _issue(
                "payroll_status",
                "PAYROLL_NOT_REVIEWED",
                f"La nómina está en estado {payroll_status}.",
                severity="WARNING",
                payroll_id=payroll.id,
            )
        )
    ...
    stored_company_total = money(getattr(payroll, "company_total_social_security", 0) or 0)
    if stored_company_total and abs(stored_company_total - values["company_component_total"]) > Decimal("0.01"):
        issues.append(
            _issue(
...
def _create_line(payroll: Payroll, selected_ccc: str) -> SocialSecuritySettlementLine:
    employee = getattr(payroll, "employee", None)
    contract = getattr(payroll, "contract", None)
...
def prepare_social_security_settlement(
    ...
    if payload.ccc_id not in valid_cccs:
        raise SocialSecuritySettlementDomainError(
            "El CCC no pertenece a la empresa ni a sus centros de trabajo"
        )
    ...
    payrolls = (
        ...
        .filter(
            ...
            Payroll.status.notin_(["cancelled", "canceled"]),
        )
    ...
    selected_payrolls = [
        payroll for payroll in payrolls if resolve_payroll_ccc(payroll) == payload.ccc_id
    ]
    ...
    for payroll in selected_payrolls:
        settlement.lines.append(_create_line(payroll, payload.ccc_id))
    ...
    settlement.status = (
        SocialSecuritySettlementStatus.VALIDATION_ERROR.value
        if _has_blocking_errors(settlement_issues + line_issues)
...
def get_social_security_settlement(
    db: Session,
    settlement_id: int,
) -> SocialSecuritySettlement | None:
    return (
        db.query(SocialSecuritySettlement)
        .options(joinedload(SocialSecuritySettlement.lines))
        .filter(SocialSecuritySettlement.id == settlement_id)
        .first()
    )
...
def list_social_security_settlements(
    ...
    *,
    ...
    query = db.query(SocialSecuritySettlement).options(
...
def confirm_social_security_settlement(
    ...
    *,
    ...
    if _has_blocking_errors(all_issues):
        raise SocialSecuritySettlementDomainError(
            "La liquidación contiene errores que impiden confirmarla."
        )

    if created_by is not None and settlement.created_by is None:
...
def _quota_payload(prefix: str, source: Any) -> dict[str, str]:
    if prefix == "employee":
        return {
            "common_contingencies": str(money(source.employee_common_contingencies)),
            "unemployment": str(money(source.employee_unemployment)),
            "training": str(money(source.employee_training)),
            "mei": str(money(source.employee_mei)),
            "total": str(money(source.employee_total)),
        }
    return {
        "common_contingencies": str(money(source.company_common_contingencies)),
        "unemployment": str(money(source.company_unemployment)),
        "fogasa": str(money(source.company_fogasa)),
        "training": str(money(source.company_training)),
        "at_ep": str(money(source.company_at_ep)),
        "mei": str(money(source.company_mei)),
        "total": str(money(source.company_total)),
    }
...
def build_simulated_settlement_content(settlement: SocialSecuritySettlement) -> str:
    payload = {
        ...
        "totals": {
            ...
            "bases": {
                "common": str(money(settlement.common_contingencies_base)),
                "professional": str(money(settlement.professional_contingencies_base)),
                "unemployment_training_fogasa": str(
                    money(settlement.unemployment_training_fogasa_base)
                ),
                "overtime": str(money(settlement.overtime_base)),
            },
            "employee_quotas": _quota_payload("employee", settlement),
            "company_quotas": _quota_payload("company", settlement),
        ...
        "workers": [
            {
                ...
                "bases": {
                    ...
                    "unemployment_training_fogasa": str(
                        money(line.unemployment_training_fogasa_base)
                    ),
                    "overtime": str(money(line.overtime_base)),
                },
                "employee_quotas": _quota_payload("employee", line),
                "company_quotas": _quota_payload("company", line),
...
def generate_social_security_settlement_file(
    ...
    *,
    ...
    if settlement.communication_file_id:
        raise SocialSecuritySettlementDomainError(
            "La liquidación ya tiene un fichero generado."
        )

    period = f"{settlement.period_year:04d}-{settlement.period_month:02d}"
    filename = (
        f"SS_{settlement.company_id}_{settlement.ccc_id}_"
        f"{settlement.period_year:04d}{settlement.period_month:02d}.json"
    )
    ...
    communication = validate_communication_file(
        db,
        communication,
        created_by=created_by,
    )
    ...
    communication = generate_communication_file(
        ...
        CommunicationFileGenerateRequest(
            ...
            metadata={
                "settlement_status": SocialSecuritySettlementStatus.GENERATED.value
            },
            created_by=created_by,
        ),
    )
...
def serialize_social_security_settlement_line(
    line: SocialSecuritySettlementLine,
) -> dict[str, Any]:
    return {
        "id": line.id,
        "payroll_id": line.payroll_id,
...
def serialize_social_security_settlement(
    settlement: SocialSecuritySettlement,
) -> dict[str, Any]:
    return {
        "id": settlement.id,
        ...
        "lines": [
            serialize_social_security_settlement_line(line)
            for line in settlement.lines
        ],
    }
