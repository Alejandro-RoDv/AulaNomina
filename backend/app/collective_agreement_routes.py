...
from app.crud.collective_agreement import (
    ...
    delete_vacation_rule,
    delete_work_time_rule,
...
def list_collective_agreements(include_inactive: bool = Query(default=False), db: Session = Depends(get_db)):
...
def create_collective_agreement_endpoint(agreement: CollectiveAgreementCreate, db: Session = Depends(get_db)):
...
def update_collective_agreement_endpoint(agreement_id: int, agreement: CollectiveAgreementUpdate, db: Session = Depends(get_db)):
...
def create_professional_group_endpoint(agreement_id: int, group: ProfessionalGroupCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
...
def update_professional_group_endpoint(group_id: int, group: ProfessionalGroupUpdate, db: Session = Depends(get_db)):
...
def create_professional_category_endpoint(agreement_id: int, category: ProfessionalCategoryCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
...
def update_professional_category_endpoint(category_id: int, category: ProfessionalCategoryUpdate, db: Session = Depends(get_db)):
    if category.professional_group_id and not get_professional_group(db, category.professional_group_id):
        raise HTTPException(status_code=404, detail="Grupo profesional no encontrado")
...
def create_salary_table_endpoint(agreement_id: int, salary_table: SalaryTableCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
...
def update_salary_table_endpoint(salary_table_id: int, salary_table: SalaryTableUpdate, db: Session = Depends(get_db)):
...
def create_salary_table_row_endpoint(salary_table_id: int, row: SalaryTableRowCreate, db: Session = Depends(get_db)):
    ensure_salary_table_exists(db, salary_table_id)
...
def update_salary_table_row_endpoint(row_id: int, row: SalaryTableRowUpdate, db: Session = Depends(get_db)):
    if row.professional_category_id and not get_professional_category(db, row.professional_category_id):
        raise HTTPException(status_code=404, detail="Categoría profesional no encontrada")
    if row.professional_group_id and not get_professional_group(db, row.professional_group_id):
        raise HTTPException(status_code=404, detail="Grupo profesional no encontrado")
...
def create_agreement_complement_endpoint(agreement_id: int, complement: AgreementComplementCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
...
def update_agreement_complement_endpoint(complement_id: int, complement: AgreementComplementUpdate, db: Session = Depends(get_db)):
...
def create_work_time_rule_endpoint(agreement_id: int, rule: WorkTimeRuleCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
    return create_work_time_rule(db, agreement_id, rule)


@router.put("/work-time-rules/{rule_id}", response_model=WorkTimeRuleResponse)
def update_work_time_rule_endpoint(rule_id: int, rule: WorkTimeRuleUpdate, db: Session = Depends(get_db)):
...
@router.delete("/work-time-rules/{rule_id}")
def delete_work_time_rule_endpoint(rule_id: int, db: Session = Depends(get_db)):
    deleted_rule = delete_work_time_rule(db, rule_id)
    if not deleted_rule:
        raise HTTPException(status_code=404, detail="Regla de jornada no encontrada")
...
def create_vacation_rule_endpoint(agreement_id: int, rule: VacationRuleCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
    return create_vacation_rule(db, agreement_id, rule)


@router.put("/vacation-rules/{rule_id}", response_model=VacationRuleResponse)
def update_vacation_rule_endpoint(rule_id: int, rule: VacationRuleUpdate, db: Session = Depends(get_db)):
...
@router.delete("/vacation-rules/{rule_id}")
def delete_vacation_rule_endpoint(rule_id: int, db: Session = Depends(get_db)):
    deleted_rule = delete_vacation_rule(db, rule_id)
    if not deleted_rule:
        raise HTTPException(status_code=404, detail="Regla de vacaciones no encontrada")
    return {"ok": True, "deleted_id": rule_id}
...
def create_leave_rule_endpoint(agreement_id: int, rule: LeaveRuleCreate, db: Session = Depends(get_db)):
    ensure_agreement_exists(db, agreement_id)
    return create_leave_rule(db, agreement_id, rule)


@router.put("/leave-rules/{rule_id}", response_model=LeaveRuleResponse)
def update_leave_rule_endpoint(rule_id: int, rule: LeaveRuleUpdate, db: Session = Depends(get_db)):
...
@router.delete("/leave-rules/{rule_id}")
def delete_leave_rule_endpoint(rule_id: int, db: Session = Depends(get_db)):
    deleted_rule = delete_leave_rule(db, rule_id)
    if not deleted_rule:
        raise HTTPException(status_code=404, detail="Regla de permiso no encontrada")
    return {"ok": True, "deleted_id": rule_id}
