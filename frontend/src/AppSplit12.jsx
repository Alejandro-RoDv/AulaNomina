...
function buildDocumentUpdatePayload(form) {
  return {
    center_id: form.center_id ? Number(form.center_id) : null,
    document_type: form.document_type,
    document_name: form.document_name,
...
export default function AppSplit12() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [contractsData, employeesData, companiesData, workCentersData, incidentsData, payrollsData, documentsData, nextEmployeeCodeData] = await Promise.all([
        fetchContracts(),
        fetchAllEmployees(),
        fetchCompanies(),
        fetchWorkCenters(),
      ...
      setDocumentError("Error cargando datos");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDocumentChange = (event) => {
    ...
    setDocumentForm((prev) => {
      if (name === "employee_id") {
        const selectedEmployee = employees.find((employee) => String(employee.id) === String(value));
        return {
          ...prev,
          employee_id: value,
          company_id: selectedEmployee?.company_id ? String(selectedEmployee.company_id) : "",
          center_id: selectedEmployee?.center_id ? String(selectedEmployee.center_id) : "",
        };
      }
  ...
  const handleUpdateDocument = async (documentId, form) => {
    setDocumentError("");
    setDocumentSuccess("");
    try {
      setDocumentSubmitting(true);
      await updateDocument(documentId, buildDocumentUpdatePayload(form));
  ...
  const handleDeleteDocument = async (documentId) => {
    ...
    try {
      ...
      setDocumentSuccess("Documento marcado como no aplica");
      await loadData();
    } catch (err) {
      setDocumentError(err.message || "Error al marcar documento");
      throw err;
    } finally {
      setDocumentSubmitting(false);
    }
  ...
  function getSubtitle() {
    ...
    if (activePage === "documents") return "Control documental laboral y expedientes de trabajador";
    return "";
  }
