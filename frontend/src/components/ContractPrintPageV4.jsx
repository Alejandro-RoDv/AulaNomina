...
const TEMPORARY_CLAUSES = [
  ...
  { key: "active_policies", label: "Programas de políticas activas de empleo", page: 10, codes: ["405", "505"], terms: ["politicas activas"] },
  { key: "eu_funds", label: "Programas financiados con fondos europeos", page: 11, codes: ["406", "506"], terms: ["fondos europeos"] },
  { key: "university", label: "Personal docente e investigador de universidades", page: 14, codes: ["409", "509"], terms: ["universidad"] },
...
function getModelPageNumber(page, contract) {
  ...
  if (page.type === "temporaryClauses" || page.type === "fixedDiscontinuous" || page.type === "indefiniteWork") return 2;
  if (page.type === "temporaryLegal" || page.type === "indefiniteClauses") return 3;
...
function openPrintWindow(html) {
  ...
  printWindow.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8" /><title>Impresión contratos</title><style>${printStyles}</style></head><body>${html}<script>window.addEventListener('load',()=>setTimeout(()=>{window.focus();window.print();},200));</script></body></html>`);
  printWindow.document.close();
}

export default function ContractPrintPageV4({ loading, contracts, employees, companies, workCenters = [] }) {
  ...
  const selectedContracts = selectedIds.map((id) => contracts.find((contract) => String(contract.id) === String(id))).filter(Boolean).sort(sortContracts);
  ...
  const handlePrint = () => {
    ...
    const html = document.getElementById("contract-print-source")?.innerHTML || "";
