        ...
        <table className="payroll-sheet__preview-table">
          ...
          <tbody>
            {lines.map((line) => {
              ...
              const unitPrice = Number(line.unit_price || 0) || (quantity ? Number(line.amount || 0) / quantity : Number(line.amount || 0));
              return (
...
function CatalogModal({ concepts, families, includedConceptIds, targetRowId, onSelect, onClose }) {
  ...
  const filtered = useMemo(() => {
    ...
    return concepts.filter((concept) => {
      ...
      return [concept.code, concept.name, concept.category, concept.salary_nature, conceptFamily(concept)]
        .filter(Boolean)
  ...
  return (
    <div className="payroll-sheet__overlay" role="dialog" aria-modal="true" aria-label="Catálogo de conceptos">
      <section className="payroll-sheet__catalog-modal">
        <header>
          <div>
            <span>CATÁLOGO DE CÓDIGOS</span>
            <h2>Buscar concepto de nómina</h2>
            <p>Filtra por código, nombre o familia y selecciónalo para rellenar la fila activa.</p>
          </div>
          <button type="button" className="payroll-sheet__close" onClick={onClose} aria-label="Cerrar">×</button>
        </header>
        ...
        <div className="payroll-sheet__catalog-filters">
          <label>
            ...
            <input
              ...
              placeholder="40, vacaciones, nocturnidad, dieta, IRPF..."
              autoFocus
            />
          </label>
          <label>
        ...
        <div className="payroll-sheet__catalog-table-wrap">
          <table className="payroll-sheet__catalog-table">
            <thead>
              <tr>
                <th>Código</th>
                <th>Concepto</th>
            ...
            <tbody>
              {filtered.map((concept) => {
                ...
                return (
                  <tr key={concept.id}>
                    <td className="is-code">{concept.code}</td>
                    ...
                    <td className="is-action">
                      <button
                        type="button"
                        disabled={included}
                        onClick={() => onSelect(targetRowId, concept)}
                      >
...
export default function MonthlyPayrollPreparationV3({ companies = [], workCenters = [], onPrepared }) {
  ...
  const conceptByCode = useMemo(() => {
    const map = new Map();
    sortedConcepts.forEach((concept) => {
      map.set(String(concept.code || "").trim().toLocaleUpperCase("es"), concept);
    });
    return map;
  }, [sortedConcepts]);
  ...
  const resolveCode = async (rowId, rawCode) => {
    const normalized = String(rawCode || "").trim().toLocaleUpperCase("es");
    setDraftRows((previous) => previous.map((row) => row.tempId === rowId ? { ...row, codeInput: rawCode, error: "" } : row));
    if (!normalized) {
      setDraftRows((previous) => previous.map((row) => row.tempId === rowId ? {
        ...makeBlankRow(0),
        tempId: row.tempId,
    ...
    const concept = conceptByCode.get(normalized);
    if (!concept) {
      setDraftRows((previous) => previous.map((row) => row.tempId === rowId ? {
        ...row,
        concept_id: null,
    ...
    <div className="payroll-sheet">
      ...
      {!ready && (
        <section className="payroll-sheet__empty">
          ...
          <span>La hoja de conceptos aparecerá debajo y podrás trabajar con ella como una matriz de nómina.</span>
        </section>
      )}
      ...
      {preparation && (
        <section className="payroll-sheet__workspace">
          ...
          <div className="payroll-sheet__rule">
            <strong>Introduce el código directamente o búscalo en el catálogo.</strong>
          ...
          <div className="payroll-sheet__grid-head">
            <div>
              ...
              <p>Las filas cargadas proceden del contrato, convenio, incidencias y cálculos automáticos. Las filas vacías son para introducir variaciones del mes.</p>
            </div>
            <button
              type="button"
                ...
                {draftRows.map((row) => (
                  <tr key={row.tempId} className={row.concept_id ? "is-new" : "is-blank"}>
                    <td className={`cell-code-input ${row.error ? "has-error" : ""}`}>
                      <div className="payroll-sheet__code-entry">
                        <input
                          ...
                          value={row.codeInput}
                          placeholder="Código"
                          onChange={(event) => setDraftRows((previous) => previous.map((item) => item.tempId === row.tempId ? { ...item, codeInput: event.target.value, error: "" } : item))}
                    ...
                    <td className="cell-concept">
                      ...
                      ) : <span className="payroll-sheet__placeholder">Escribe un código o usa el buscador</span>}
                    </td>
          ...
          <div className="payroll-sheet__row-tools">
            ...
            <span>{sortedConcepts.length} códigos disponibles en el catálogo</span>
          </div>
