...
function calculatePreview(form) {
  ...
  const extraPay = Number(form.extra_pay_proration || 0);
...
export default function PayrollForm({
  ...
  return (
    <form onSubmit={onSubmit} style={styles.form}>
          ...
          >
            ...
            {availableContracts.map((contract) => (
              <option key={contract.id} value={contract.id}>
                #{contract.id} · {contract.contract_type} · {contract.start_date} · {formatCurrency(contract.salary_base || 0)}
              </option>
            ))}
          </select>
        </div>
      ...
      <div style={styles.formRow}>
        ...
        <div style={styles.formGroup}>
          ...
          <input
            type="number"
            step="0.01"
            name="salary_supplements"
            value={form.salary_supplements}
            onChange={onChange}
            min="0"
            style={styles.input}
          />
        ...
        <div style={styles.formGroup}>
          ...
          <input
            type="number"
            step="0.01"
            name="extra_pay_proration"
            value={form.extra_pay_proration}
            onChange={onChange}
            min="0"
            style={styles.input}
          />
      ...
      {form.employee_id && availableContracts.length === 0 && (
...
export { PAYROLL_STATUS_OPTIONS, MONTH_OPTIONS, formatCurrency };
...
const styles = {
  ...
  previewPanel: { flex: 1, minWidth: "360px", display: "grid", gridTemplateColumns: "repeat(4, minmax(90px, 1fr))", gap: "8px", border: "1px solid #e5e7eb", borderRadius: "10px", backgroundColor: "#f9fafb", padding: "10px" },
