...
export default function IncidentContributionControl({
  ...
  return <section className="incident-engine-grid">
    <article className="incident-engine-panel">
      ...
      <div className="incident-base-list">
        {BASE_ROWS.map(([key, label]) => <div key={key}>
          ...
          <span>{money(automatic[key])}<small>cálculo automático</small></span>
          <b>{money(resolved[key])}<small>base aplicada</small></b>
        </div>)}
      </div>
    </article>
    ...
    <article className="incident-engine-panel">
      <header>
        <div>
          <span className="incident-panel-kicker"><ShieldCheck size={15} /> Ajuste manual</span>
          <h3>Bases informadas manualmente</h3>
        </div>
      </header>
      <form className="incident-override-form" onSubmit={onSubmit}>
        ...
        <p>Deja el campo vacío para mantener el cálculo automático. Al aplicar un ajuste se genera una nueva versión del cálculo.</p>
        <div>
          <button type="button" onClick={onClear} className="incident-button-quiet">Restablecer campos</button>
          <button type="submit" disabled={saving} className="incident-button-primary">{saving ? "Guardando…" : "Aplicar y recalcular"}</button>
        </div>
      </form>
    </article>
  </section>;
