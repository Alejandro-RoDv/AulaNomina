...
const SEGMENT_LABELS = {
  ...
  medical_leave: "Incapacidad temporal",
  absence: "Ausencia",
  vacation: "Vacaciones",
  overtime: "Horas extraordinarias",
  movement: "Variación laboral",
};
...
const ADJUSTMENT_LABELS = {
  salary_supplements: "Complementos salariales",
  seniority_amount: "Antigüedad",
  variable_incentives: "Variables e incentivos",
  extra_pay_proration: "Prorrata de pagas extra",
};

function segmentLabel(value) {
  return SEGMENT_LABELS[value] || String(value || "—").replaceAll("_", " ");
}

function contributionLabel(value) {
  return CONTRIBUTION_LABELS[value] || String(value || "—").replaceAll("_", " ");
}

function adjustmentLabel(value) {
  return ADJUSTMENT_LABELS[value] || String(value || "—").replaceAll("_", " ");
}
...
export function IncidentSegmentTable({ segments = [] }) {
  return <section className="incident-engine-panel incident-segments-panel">
    <header>
      <div>
        <span className="incident-panel-kicker"><Eye size={15} /> Explicación del cálculo</span>
        <h3>Segmentos del periodo</h3>
        <p>Cada tramo muestra el tratamiento salarial y de cotización aplicado.</p>
      </div>
      <strong>{segments.length} {segments.length === 1 ? "segmento" : "segmentos"}</strong>
    </header>
    <div className="incident-segment-table-wrap"><table className="incident-segment-table">
      ...
      <tbody>
        {segments.map((segment) => <tr key={segment.segment_key}>
          ...
          <td><strong>{segmentLabel(segment.segment_type)}</strong><small>{segment.incident_id ? `Incidencia ${segment.incident_id}` : "Periodo ordinario"}</small></td>
          ...
          <td><span className={`incident-source-pill ${segment.contribution_treatment}`}>{contributionLabel(segment.contribution_treatment)}</span></td>
        </tr>)}
...
export function IncidentComponentAdjustments({ adjustments = [] }) {
  return <section className="incident-engine-panel">
    <header>
      <div>
        <span className="incident-panel-kicker"><Calculator size={15} /> Conceptos sensibles</span>
        <h3>Ajustes aplicados</h3>
      </div>
      <strong>{adjustments.length} {adjustments.length === 1 ? "concepto" : "conceptos"}</strong>
    </header>
    <div className="incident-adjustment-grid">
      {adjustments.map((item) => <article key={item.field}>
        <span>{adjustmentLabel(item.field)}</span>
