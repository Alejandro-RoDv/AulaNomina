...
export default function ContractExtraPaySimulationPanel({ agreement }) {
  ...
  const table = useMemo(() => tables.find((item) => String(item.id) === String(tableId)) || initialTable, [tables, tableId, initialTable]);
  const selectedPay = useMemo(() => pays.find((item) => String(item.id) === String(payId)) || pays[0] || null, [pays, payId]);
  const eligibleContracts = useMemo(() => contracts
    .filter((item) => Number(item.collective_agreement_id) === Number(agreement?.id))
    .filter((item) => !["deleted", "cancelled"].includes(item.status))
    .sort((a, b) => String(a.employee_name || "").localeCompare(String(b.employee_name || ""))), [contracts, agreement?.id]);
  const selectedContract = useMemo(() => eligibleContracts.find((item) => String(item.id) === String(contractId)) || eligibleContracts[0] || null, [eligibleContracts, contractId]);
  ...
  useEffect(() => {
    if (!open) return;
    fetchContracts().then((data) => setContracts(data || [])).catch((err) => setError(err.message));
  }, [open, agreement?.id]);
  ...
  return <section style={s.box}>
    <header style={s.head}>
      <div><h3 style={s.title}>Cálculo por contrato</h3><p style={s.sub}>Ajusta la paga por vigencia, jornada, incidencias e inactividad y genera el período especial.</p></div>
    ...
    {open && <div style={s.body}>
      ...
      {selectedPay && rules && <section style={s.card}>
        <div style={s.cardHead}><div><h4 style={s.cardTitle}>Reglas de devengo</h4><p style={s.sub}>Estas reglas pertenecen a {selectedPay.name}.</p></div><button type="button" onClick={saveRules} disabled={busy} style={s.light}>Guardar reglas</button></div>
        <div style={s.rules}>
      ...
      <div style={s.actions}><button type="button" onClick={calculate} disabled={busy || !selectedPay || !selectedContract} style={s.yellow}>Calcular vista previa</button></div>
      ...
      {preview && <>
        ...
        <section style={s.card}>
          ...
          <div style={s.scroll}><table style={s.table}><thead><tr><th>Concepto</th><th>Origen</th><th>Base contrato</th><th>Jornada completa</th><th>Con parcialidad</th><th>Devengado</th></tr></thead><tbody>{preview.lines.map((line) => <tr key={line.concept_line_id}><td><strong>{line.concept_name}</strong></td><td>{SOURCE_LABELS[line.base_source] || line.base_source}</td><td>{money(line.contract_base_amount)}</td><td>{money(line.full_time_amount)}</td><td>{money(line.after_partiality_amount)}</td><td><strong>{money(line.final_amount)}</strong></td></tr>)}</tbody></table></div>
        </section>

        <section style={s.card}>
          <h4 style={s.cardTitle}>Incidencias durante el devengo</h4>
          <div style={s.scroll}><table style={s.table}><thead><tr><th>Tipo</th><th>Período</th><th>Días coincidentes</th><th>Días descontados</th><th>Tratamiento</th></tr></thead><tbody>{preview.incident_breakdown.map((item) => <tr key={`${item.incident_id}-${item.start_date}`}><td>{item.label}</td><td>{dateLabel(item.start_date)} – {dateLabel(item.end_date)}</td><td>{item.overlapping_days}</td><td>{item.deducted_days}</td><td>{item.deducted ? "Descuenta devengo" : "No descuenta"}</td></tr>)}{!preview.incident_breakdown.length && <tr><td colSpan="5">Sin incidencias en el período.</td></tr>}</tbody></table></div>
        </section>
      ...
      {result && <section style={s.result}><Stat label="Nómina" value={`#${result.payroll_id}`} /><Stat label="Bruto" value={money(result.gross_salary)} /><Stat label="IRPF" value={`${percent(result.irpf_percentage)} · ${money(result.irpf)}`} /><Stat label="Neto" value={money(result.net_salary)} /></section>}
    </div>}
  </section>;
}

function Field({ label, children }) { return <label style={s.field}><span>{label}</span>{children}</label>; }
function Check({ label, checked, onChange }) { return <label style={s.check}><input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} />{label}</label>; }
function Stat({ label, value, strong }) { return <div style={s.stat}><span>{label}</span><strong style={strong ? { fontSize: "16px" } : undefined}>{value}</strong></div>; }

const cell = { padding: "7px", borderBottom: "1px solid #e5e7eb", textAlign: "left", fontSize: "11px", verticalAlign: "top" };
const s = {
  box: { border: "1px solid #d1d5db", background: "#fff" }, head: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", background: "#f9fafb" }, title: { margin: 0, fontSize: "15px", fontWeight: 850 }, sub: { margin: "3px 0 0", color: "#6b7280", fontSize: "11px" }, body: { borderTop: "1px solid #e5e7eb", padding: "14px", display: "flex", flexDirection: "column", gap: "10px" }, filters: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: "9px" }, card: { border: "1px solid #e5e7eb", padding: "12px" }, cardHead: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "9px" }, cardTitle: { margin: "0 0 8px", fontSize: "13px", fontWeight: 850 }, rules: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: "8px", alignItems: "end" }, field: { display: "flex", flexDirection: "column", gap: "5px", fontSize: "11px", fontWeight: 800 }, input: { width: "100%", height: "34px", boxSizing: "border-box", border: "1px solid #d1d5db", background: "#fff", padding: "6px 8px" }, check: { minHeight: "34px", display: "flex", alignItems: "center", gap: "7px", fontSize: "11px", fontWeight: 750 }, stats: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(145px, 1fr))", gap: "8px" }, stat: { border: "1px solid #e5e7eb", padding: "9px", display: "flex", flexDirection: "column", gap: "2px", fontSize: "11px" }, actions: { display: "flex", justifyContent: "flex-end" }, generate: { display: "grid", gridTemplateColumns: "180px 180px auto", gap: "9px", alignItems: "end", padding: "11px", border: "1px solid #d1d5db", background: "#f9fafb" }, result: { display: "grid", gridTemplateColumns: "repeat(4, minmax(120px, 1fr))", gap: "8px", padding: "10px", border: "1px solid #bbf7d0", background: "#f0fdf4" }, scroll: { overflowX: "auto" }, table: { width: "100%", minWidth: "760px", borderCollapse: "collapse", fontSize: "11px" }, dark: { height: "34px", border: "1px solid #111827", background: "#111827", color: "#fff", padding: "0 12px", fontSize: "11px", fontWeight: 850, cursor: "pointer" }, light: { height: "34px", border: "1px solid #d1d5db", background: "#fff", padding: "0 11px", fontSize: "11px", fontWeight: 800, cursor: "pointer" }, yellow: { height: "34px", border: "1px solid #eab308", background: "#facc15", padding: "0 12px", fontSize: "11px", fontWeight: 850, cursor: "pointer" }, disabled: { height: "34px", border: "1px solid #e5e7eb", background: "#f3f4f6", color: "#9ca3af", padding: "0 12px", fontSize: "11px", fontWeight: 850 }, error: { padding: "9px", border: "1px solid #fecaca", background: "#fef2f2", color: "#991b1b", fontSize: "11px" }, ok: { padding: "9px", border: "1px solid #bbf7d0", background: "#f0fdf4", color: "#166534", fontSize: "11px" }, notice: { padding: "9px", border: "1px solid #e5e7eb", background: "#f9fafb", fontSize: "11px" }, warning: { padding: "9px", border: "1px solid #fde68a", background: "#fffbeb", color: "#78350f", display: "flex", flexDirection: "column", gap: "3px", fontSize: "11px" }, block: { padding: "9px", border: "1px solid #fecaca", background: "#fef2f2", color: "#991b1b", fontSize: "11px" },
};
Object.assign(s.table, { "& th": cell, "& td": cell });
