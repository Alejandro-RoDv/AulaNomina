...
function cleanPayload(payload) {
  return Object.fromEntries(Object.entries(payload).filter(([key]) => !["id", "table_id", "table_name", "table_year"].includes(key)).map(([key, value]) => [key, value === "" ? null : value]));
}
...
export default function CollectiveAgreementsPage({ loading, collectiveAgreements = [], onDataChanged }) {
  ...
  useEffect(() => {
    ...
    fetchCollectiveAgreement(activeAgreement.id)
      .then((data) => { setSelectedAgreement(data); setError(""); })
  ...
  return (
    <div style={styles.wrapper}>
      ...
      {selectedAgreement && activeTab === "overview" && <div style={styles.overviewLayout}><Section title="Ficha administrativa" subtitle="Resumen del expediente de convenio."><DefinitionTable rows={[["Nombre", selectedAgreement.name], ["Código oficial", selectedAgreement.agreement_code || "—"], ["Sector", selectedAgreement.sector || "—"], ["Ámbito territorial", selectedAgreement.territorial_scope || "—"], ["Vigente desde", formatDate(selectedAgreement.effective_from)], ["Vigente hasta", formatDate(selectedAgreement.effective_to)], ["Estado", selectedStatus.label], ["Notas", selectedAgreement.notes || "Sin notas"]]} /></Section><Section title="Control del convenio" subtitle="Indicadores y alertas."><StatsGrid items={[["Grupos", groups.length], ["Categorías", categories.length], ["Tablas", salaryTables.length], ["Filas salariales", salaryRows.length], ["Jornada", workTimeRules.length], ["Vacaciones", vacationRules.length], ["Permisos", leaveRules.length], ["Complementos", complements.length]]} /><div style={styles.alertBlock}><h4 style={styles.alertTitle}>Alertas</h4><AlertsList alerts={alerts} /></div></Section></div>}

      {selectedAgreement && activeTab === "classification" && <Section title="Clasificación profesional" subtitle="Selecciona un grupo y gestiona sus categorías."><ActionBar actions={[["+ Nuevo grupo", () => { setGroupForm(initialGroupForm); togglePanel("group"); }, "primary"], ["+ Nueva categoría", () => { setCategoryForm(selectedGroup ? { ...initialCategoryForm, professional_group_id: selectedGroup.id } : initialCategoryForm); togglePanel("category"); }]]} />{openPanel === "group" && <InlineForm title={groupForm.id ? "Editar grupo profesional" : "Nuevo grupo profesional"} onSubmit={handleSaveGroup}><Field label="Nombre del grupo"><input style={styles.input} value={groupForm.name || ""} onChange={(e) => setGroupForm({ ...groupForm, name: e.target.value })} required /></Field><Field label="Código"><input style={styles.input} value={groupForm.code || ""} onChange={(e) => setGroupForm({ ...groupForm, code: e.target.value })} /></Field><Field label="Descripción"><textarea style={styles.textarea} value={groupForm.description || ""} onChange={(e) => setGroupForm({ ...groupForm, description: e.target.value })} /></Field><button type="submit" disabled={submitting} style={styles.primaryButton}>Guardar</button></InlineForm>}{openPanel === "category" && <InlineForm title={categoryForm.id ? "Editar categoría profesional" : "Nueva categoría profesional"} onSubmit={handleSaveCategory}><Field label="Grupo"><select style={styles.input} value={categoryForm.professional_group_id || ""} onChange={(e) => setCategoryForm({ ...categoryForm, professional_group_id: e.target.value })}><option value="">Sin grupo</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Field><Field label="Código"><input style={styles.input} value={categoryForm.code || ""} onChange={(e) => setCategoryForm({ ...categoryForm, code: e.target.value })} /></Field><Field label="Categoría"><input style={styles.input} value={categoryForm.name || ""} onChange={(e) => setCategoryForm({ ...categoryForm, name: e.target.value })} required /></Field><Field label="Nivel"><input style={styles.input} value={categoryForm.level || ""} onChange={(e) => setCategoryForm({ ...categoryForm, level: e.target.value })} /></Field><button type="submit" disabled={submitting} style={styles.primaryButton}>Guardar</button></InlineForm>}<div style={styles.classificationLayout}><section style={styles.leftPane}><div style={styles.paneTitle}>Grupos profesionales</div><SimpleTable columns={["Grupo", "Código", "Acciones"]} empty="Sin grupos registrados." rows={groups.map((group) => [<button type="button" onClick={() => setSelectedGroupId(String(group.id))} style={String(selectedGroup?.id) === String(group.id) ? styles.rowSelectActive : styles.rowSelect}>{group.name}</button>, group.code || "—", rowActions(() => { setGroupForm({ ...initialGroupForm, ...group }); setOpenPanel("group"); }, () => deleteItem(() => deleteProfessionalGroup(group.id), "grupo"))])} /></section><section style={styles.rightPane}><div style={styles.paneHeader}><div><h3 style={styles.paneHeading}>{selectedGroup ? `Categorías de ${selectedGroup.name}` : "Categorías"}</h3><p style={styles.paneSubtitle}>{filteredCategories.length} categorías en el grupo seleccionado</p></div>{selectedGroup && <button type="button" onClick={() => startCategoryForGroup(selectedGroup)} style={styles.secondaryButton}>Añadir categoría</button>}</div><SimpleTable columns={["Código", "Categoría", "Nivel", "Acciones"]} empty="Sin categorías para este grupo." rows={filteredCategories.map((category) => [category.code || "—", category.name, category.level || "—", rowActions(() => { setCategoryForm({ ...initialCategoryForm, ...category }); setOpenPanel("category"); }, () => deleteItem(() => deleteProfessionalCategory(category.id), "categoría"))])} /></section></div></Section>}

      {selectedAgreement && activeTab === "salary" && <Section title="Tablas salariales" subtitle="Tablas anuales arriba; filas salariales abajo."><ActionBar actions={[["+ Crear tabla", () => { setSalaryTableForm(initialSalaryTableForm); togglePanel("salary-table"); }, "primary"], ["+ Añadir fila", () => { setSalaryRowForm(initialSalaryRowForm); togglePanel("salary-row"); }]]} />{openPanel === "salary-table" && <SalaryTableForm form={salaryTableForm} setForm={setSalaryTableForm} onSubmit={handleSaveSalaryTable} submitting={submitting} />}{openPanel === "salary-row" && <SalaryRowForm form={salaryRowForm} setForm={setSalaryRowForm} salaryTables={salaryTables} categories={categories} onSubmit={handleSaveSalaryRow} submitting={submitting} />}<div style={styles.salaryBlock}><h3 style={styles.subsectionTitle}>Tablas del convenio</h3><SalaryTablesSummary salaryTables={salaryTables} onEdit={(table) => { setSalaryTableForm({ ...initialSalaryTableForm, ...table }); setOpenPanel("salary-table"); }} onDelete={(table) => deleteItem(() => deleteSalaryTable(table.id), "tabla salarial")} /></div><div style={styles.salaryBlock}><h3 style={styles.subsectionTitle}>Filas salariales</h3><SimpleTable columns={["Tabla", "Año", "Categoría", "Grupo", "Salario base", "Plus convenio", "Antigüedad", "Total", "Acciones"]} empty="Sin filas salariales." rows={salaryRows.map((row) => [row.table_name, row.table_year, row.category_name || "—", row.group_name || getGroupName(groups, row.professional_group_id), money(row.base_salary), money(row.agreement_plus), money(row.seniority_amount), money(row.total_amount), rowActions(() => { setSalaryRowForm({ ...initialSalaryRowForm, ...row, salary_table_id: row.table_id }); setOpenPanel("salary-row"); }, () => deleteItem(() => deleteSalaryTableRow(row.id), "fila salarial"))])} /></div></Section>}
...
const styles = {
  ...
  input: { height: "34px", padding: "6px 8px", border: "1px solid #d1d5db", borderRadius: "5px", fontSize: "13px", backgroundColor: "#fff" }, textarea: { minHeight: "70px", padding: "6px 8px", border: "1px solid #d1d5db", borderRadius: "5px", fontSize: "13px", backgroundColor: "#fff", resize: "vertical" }, fieldLabel: { display: "flex", flexDirection: "column", gap: "5px", color: "#4b5563", fontSize: "12px", fontWeight: 750 },
  ...
  tableBox: { overflowX: "auto", borderTop: "1px solid #e5e7eb" }, table: { width: "100%", borderCollapse: "collapse", fontSize: "12.5px", backgroundColor: "#fff" }, th: { textAlign: "left", padding: "8px 8px", borderBottom: "1px solid #d1d5db", backgroundColor: "#f9fafb", color: "#374151", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.03em", whiteSpace: "nowrap" }, td: { padding: "8px 8px", borderBottom: "1px solid #f3f4f6", color: "#111827", verticalAlign: "middle", whiteSpace: "nowrap" },
  salaryBlock: { padding: "12px", borderBottom: "1px solid #f3f4f6" }, ruleStack: { display: "flex", flexDirection: "column", gap: "12px", padding: "12px" }, subsectionTitle: { margin: "0 0 8px", paddingBottom: "6px", borderBottom: "1px solid #e5e7eb", fontSize: "14px", fontWeight: 850 }, formGrid: { display: "grid", gridTemplateColumns: "repeat(2, minmax(220px, 1fr))", gap: "10px" }, formGridThree: { display: "grid", gridTemplateColumns: "repeat(3, minmax(180px, 1fr))", gap: "10px" }, empty: { padding: "10px", color: "#6b7280", backgroundColor: "#fff", fontSize: "13px", fontWeight: 700 }, helpText: { margin: "12px", color: "#4b5563", fontSize: "13px" },
