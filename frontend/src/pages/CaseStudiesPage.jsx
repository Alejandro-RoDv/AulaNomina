        ...
        <form style={styles.card} onSubmit={handleCreateCase}>
          <div style={styles.cardHeader}>
            <div>
              ...
              <p style={styles.cardSubtitle}>Define el enunciado general del ejercicio. El seguimiento del avance se consulta en Progreso.</p>
            </div>
            <button type="button" style={styles.secondaryButton} onClick={handleSeedDemo} disabled={submitting}>Cargar demo</button>
          </div>

          <div style={styles.formGrid}>
            <label style={styles.field}>Título<input name="title" value={caseForm.title} onChange={handleCaseChange} required style={styles.input} /></label>
            <label style={styles.field}>Dificultad<select name="difficulty" value={caseForm.difficulty} onChange={handleCaseChange} style={styles.input}><option value="basic">Básico</option><option value="intermediate">Medio</option><option value="advanced">Avanzado</option></select></label>
            <label style={styles.field}>Estado<select name="status" value={caseForm.status} onChange={handleCaseChange} style={styles.input}><option value="draft">Borrador</option><option value="active">Activo</option><option value="archived">Archivado</option></select></label>
            <label style={styles.field}>Creado por<input name="created_by" value={caseForm.created_by} onChange={handleCaseChange} style={styles.input} /></label>
          </div>

          <label style={styles.field}>Descripción<textarea name="description" value={caseForm.description} onChange={handleCaseChange} rows={3} style={styles.textarea} /></label>
          <button type="submit" style={styles.primaryButton} disabled={submitting}>Crear caso</button>
        </form>
      </section>
            ...
            {cases.map((caseStudy) => {
              ...
              return (
                <button key={caseStudy.id} type="button" onClick={() => setSelectedCaseId(caseStudy.id)} style={{ ...styles.caseButton, ...(isSelected ? styles.caseButtonActive : {}) }}>
                  ...
                  <span style={styles.caseMeta}>{difficultyLabels[caseStudy.difficulty]} · {caseStatusLabels[caseStudy.status]} · {total} tareas</span>
                </button>
              );
            })}
          </div>
            ...
            <>
              <div style={styles.detailHeader}>
                ...
                <div style={styles.headerActions}>
                  ...
                  <button type="button" style={styles.dangerButton} onClick={() => handleDeleteCase(selectedCase.id)}>Eliminar</button>
                </div>
              </div>
              ...
              <form style={styles.taskForm} onSubmit={handleCreateTask}>
                ...
                <div style={styles.formGrid}>
                  <label style={styles.field}>Título<input name="title" value={taskForm.title} onChange={handleTaskChange} required style={styles.input} /></label>
                  <label style={styles.field}>Módulo<select name="module" value={taskForm.module} onChange={handleTaskChange} style={styles.input}>{Object.entries(moduleLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                  <label style={styles.field}>Orden<input type="number" name="task_order" value={taskForm.task_order} onChange={handleTaskChange} min="1" style={styles.input} /></label>
                </div>
                <label style={styles.field}>Descripción<textarea name="description" value={taskForm.description} onChange={handleTaskChange} rows={2} style={styles.textarea} /></label>
                <label style={styles.field}>Resultado esperado<textarea name="expected_result" value={taskForm.expected_result} onChange={handleTaskChange} rows={2} style={styles.textarea} /></label>
                <label style={styles.checkboxField}><input type="checkbox" name="is_required" checked={taskForm.is_required} onChange={handleTaskChange} />Tarea obligatoria</label>
                <button type="submit" style={styles.primaryButton} disabled={submitting}>Añadir tarea</button>
              </form>
            </>
          ) : (
...
const styles = {
  ...
  topGrid: { display: "grid", gridTemplateColumns: "1fr", gap: "18px" },
